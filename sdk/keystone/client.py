...
class Action:
    ...
    workspace_id / connection_id are optional. When set, the action is linked
    to that workspace / connection so it shows up in workspace stats, audit
    filters, and connection usage counts.
    """
    tool: str = "servicenow"
...
class Keystone:
    ...
    # Default points to the hosted Keystone instance.
    # Override for local dev: Keystone(base_url="http://localhost:8000", ...)
    HOSTED_URL = "https://api.keystonegovernance.com"
    ...
    def execute(self, action_id: str) -> KeystoneResult:
        """
        Convert an observe_only action into a real enforced run.
        Call this after ks.run(..., mode="observe_only") to run for real.
        """
        resp = self._session.post(f"{self.base_url}/v1/actions/{action_id}/execute-from-dry-run")
        resp.raise_for_status()
        return KeystoneResult(resp.json())
