...
class ServiceNowSimulator(BaseConnector):
    ...
    def execute_update(self, sys_ids: list[str], changes: dict, metadata: dict | None = None) -> list[dict]:
        ...
        This is NOT in the intended changes, so "only_intended_fields"
        will detect it and trip the breaker (contained scenario).
        ...
        for sid in sys_ids:
            ...
            before_snapshot = {f: rec.get(f) for f in changes.keys()}

            # Track fields that actually change (excluding work_notes attribution)
            fields_changed = []
            unexpected = {}
            ...
            # Business rule: state → "resolved" auto-populates resolved_at.
            # resolved_at IS in changes_applied so the safety check catches it.
