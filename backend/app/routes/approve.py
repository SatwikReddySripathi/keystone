...
Approvals are permission-scoped:
  - Admins can approve any action.
  - Non-admins can only approve actions where they own the agent, OR
    they are a member (admin/approver role) of the action's workspace.
  - In every case, their authorized_tools list must include the action's tool.
"""
import json
...
from app.routes.auth import can_approve_action
...
def approve_action(
    ...
    """Approve an action. Employee must be admin, agent owner, or workspace approver."""
    with get_db() as conn:
        action = conn.execute(
        ...
        # Permission check: admin OR agent owner OR workspace admin/approver
        allowed, reason = can_approve_action(conn, org_id, body.employee_id, action_id)
        if not allowed:
            raise HTTPException(403, f"Cannot approve: {reason}")

        # Full approver identity for the receipt
...
def deny_action(
    ...
    """Deny an action. Same permission model as approve."""
    with get_db() as conn:
        action = conn.execute(
            "SELECT * FROM actions WHERE action_id=? AND org_id=?",
            (action_id, org_id)
        ...
        # Same permission model as approve
        allowed, reason = can_approve_action(conn, org_id, body.employee_id, action_id)
        if not allowed:
            raise HTTPException(403, f"Cannot deny: {reason}")
