...
def _resolve_slack_approver(conn, slack_user_id: str, slack_email: str | None) -> dict | None:
    ...
    Resolution order:
      1. approvers.slack_user_id exact match (canonical mapping once configured)
      2. approvers.email lowercase match against the Slack profile email
         (requires the Slack app to have `users:read.email` scope)

    Returns the employee row as a dict, or None if no mapping exists.
    Null = the Slack user has no identity on our side and cannot approve.
    """
    if slack_user_id:
        row = conn.execute(
            """SELECT employee_id, name, email, designation, department,
                      authorized_tools, is_admin, active
               FROM approvers WHERE slack_user_id = ? AND active = 1""",
            (slack_user_id,)
        ).fetchone()
        if row:
            return dict(row)
    ...
    if slack_email:
        row = conn.execute(
            """SELECT employee_id, name, email, designation, department,
                      authorized_tools, is_admin, active
               FROM approvers WHERE lower(email) = lower(?) AND active = 1""",
            (slack_email,)
        ).fetchone()
        if row:
            return dict(row)
...
async def slack_interact(request: Request):
    ...
    slack_user_name = user.get("real_name", user.get("username", "Unknown"))
    slack_user_email = user.get("email")  # requires `users:read.email` scope
...
def _action_org(conn, action_id: str) -> str:
    row = conn.execute("SELECT org_id FROM actions WHERE action_id = ?", (action_id,)).fetchone()
