...
from app.connectors.email_generic import get_connector as get_email
...
def run_action(body: ActionInput, request: Request, org_id: str = Depends(authenticate)):
    """
    ...
    with get_db() as conn:
        ...
        # Validate workspace_id / connection_id belong to this org
        if body.workspace_id:
            ...
            if not ws:
                raise HTTPException(400, f"Unknown workspace: {body.workspace_id}")
        if body.connection_id:
            c = conn.execute(
                "SELECT connection_id FROM connections WHERE connection_id=? AND org_id=?",
                (body.connection_id, org_id)
            ).fetchone()
            if not c:
                raise HTTPException(400, f"Unknown connection: {body.connection_id}")

        # Agent registry validation.
        #
        # Auto-registration: if an SDK client sends an unknown actor.id, we
        # create a stub agent row with status='pending_registration'. This
        # surfaces it in the /agents pane so an admin can assign workspace,
        ...
        if body.actor and body.actor.id:
                ...
                conn.execute(
                    """INSERT INTO agents
                       (agent_id, org_id, workspace_id, name, description,
                        owner_employee_id, status, permissions_json, last_used_at)
                       VALUES (?,?,?,?,?,?,?,?,datetime('now'))""",
                    (body.actor.id, org_id, body.workspace_id,
                     body.actor.name or body.actor.id,
                     "Auto-registered from SDK — pending admin review",
                     None, "pending_registration",
                     json.dumps({"tools": [], "action_types": []}))
                )
            ...
            if agent["status"] == "revoked":
                raise HTTPException(403, f"Agent {body.actor.id} is revoked")
            if agent["status"] == "paused":
                raise HTTPException(403, f"Agent {body.actor.id} is paused — resume before running")

            # Tool + workspace checks (skip for pending agents so they can be
            # discovered even if they try a tool that's not yet in their permission list)
            if agent["status"] != "pending_registration":
                perms = {}
                try:
                    perms = json.loads(agent["permissions_json"] or "{}")
                except (json.JSONDecodeError, TypeError):
                    pass
                allowed_tools = perms.get("tools") or []
                if allowed_tools and body.tool not in allowed_tools:
                    raise HTTPException(
                        403,
                        f"Agent {body.actor.id} not authorized for tool '{body.tool}'. "
                        f"Allowed: {allowed_tools}"
                    )
                if agent["workspace_id"] and body.workspace_id and agent["workspace_id"] != body.workspace_id:
                    raise HTTPException(
                        403,
                        f"Agent {body.actor.id} belongs to workspace {agent['workspace_id']}, "
                        f"cannot run in {body.workspace_id}"
                    )

            conn.execute(
                "UPDATE agents SET last_used_at = datetime('now') WHERE agent_id = ?",
                (body.actor.id,)
            )
        ...
        conn.execute(
            """INSERT INTO actions
               (action_id, org_id, status, tool, action_type,
                ...
                client_ip, workspace_id, connection_id)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (action_id, org_id, "pending", body.tool, body.action_type,
             ...
             client_ip, body.workspace_id, body.connection_id)
        )
        add_event(conn, action_id, "action.created", {"mode": body.mode})

        if auto_registered_now:
            add_event(conn, action_id, "agent.auto_registered", {
                "agent_id": body.actor.id,
                "source": "sdk",
        ...
        # Resolve which policy applies: agent override > workspace > default.
        policy_file = _resolve_policy_file(
            conn, org_id, body.actor.id if body.actor else None, body.workspace_id
        )
        decision = evaluate_policy(preview, approval_dict, policy_file=policy_file)
        add_event(conn, action_id, "policy.resolved", {
...
def execute_from_dry_run(action_id: str, org_id: str = Depends(authenticate)):
    ...
    with get_db() as conn:
        # Create new action — inherit workspace/connection from parent dry-run
        conn.execute(
            """INSERT INTO actions
               (action_id, org_id, status, tool, action_type,
                environment, actor_json, params_json, idempotency_key, mode,
            ...
            (new_action_id, org_id, "pending", body.tool, body.action_type,
             ...
             action["workspace_id"] if "workspace_id" in action.keys() else None,
             action["connection_id"] if "connection_id" in action.keys() else None)
        )
...
def _resolve_policy_file(conn, org_id: str, agent_id: str | None, workspace_id: str | None) -> str:
    """
    Pick which policy YAML file to evaluate an action against.

    Precedence (strictest match first):
      1. Agent.policy_id (if agent is registered and has explicit override)
      2. Workspace.policy_id (if workspace has a bound policy)
      3. The policy with is_default = 1
      4. Fallback to 'default_policy.yaml' if nothing else resolves

    Returns the source_file name (e.g. "strict_policy.yaml") to pass to evaluate_policy.
    """
    pid = None

    if agent_id:
        row = conn.execute(
            "SELECT policy_id FROM agents WHERE agent_id = ? AND org_id = ?",
            (agent_id, org_id)
        ).fetchone()
        if row and row["policy_id"]:
            pid = row["policy_id"]

    if not pid and workspace_id:
        row = conn.execute(
            "SELECT policy_id FROM workspaces WHERE workspace_id = ? AND org_id = ?",
            (workspace_id, org_id)
        ).fetchone()
        if row and row["policy_id"]:
            pid = row["policy_id"]
    ...
    if not pid:
        row = conn.execute(
            "SELECT policy_id FROM policies WHERE org_id = ? AND is_default = 1 LIMIT 1",
            (org_id,)
        ).fetchone()
        if row:
            pid = row["policy_id"]
    ...
    if pid:
        row = conn.execute(
            "SELECT source_file FROM policies WHERE policy_id = ? AND org_id = ?",
            (pid, org_id)
        ).fetchone()
        if row and row["source_file"]:
            return row["source_file"]

    return "default_policy.yaml"
